...
static constexpr bool sensanal = false;
...
class czseir :
#ifdef NEW
   public estimableseirfilter<true>
#else
        public seirmodel
#endif
{
...
public:
    ...
    czseir() : estimableseirfilter(eseirnumstates,3, {20,20,10}) {}
    virtual dmatrix F(unsigned , const vector<double>& , const G& ) const
    {
        return getH();
    }
...
//#endif
    ...
    vector<double>
#endif
         I(unsigned  t, const vector<double>& params, const G& g) const
    {
...
//                case eseirqe:
    ...
    virtual double contrastaddition(
#ifdef NEW
            const vector<double>& /* params */,
#endif
            const evalresult& r) const
    {
        unsigned t = 70;
    ...
    vector<double>
#endif
    X0(const vector<double>& , const G&) const
    {
...
double preiota = // exp(-p[eomega0] - p[eomegag]*g.z[t1][egammared] - p[eomegas]*g.z[t1][esred]);
...
#ifdef IMUNITY
#ifdef NEW
        iot *= (1-p[eimunity] * max(numantibodiesv(g.est[at].x()) / 10693000.00,0.0));
#else
    ...
    double weekavea(unsigned t, const history& g) const
    {
        unsigned former = t>7 ? t-7 : 0;
        double numrep;
        if(t == 0)
        ...
        {
            numrep = (g.y[t][eRA]-g.y[former][eRA])/ (t-former);
        }
        return numrep;
    }


    double weekaves(unsigned t, const history& g) const
    {
        unsigned former = t>7 ? t-7 : 0;
        double numrep;
        if(t == 0)
        ...
        {
            numrep = (g.y[t][eRS]-g.y[former][eRS])/ (t-former);
        }
        return numrep;
    }
...
template <typename S>
void fillsi(S& si, csv<','>&c, unsigned lag=0)
{
    bool rfin = false;
...
void seir()
{
    ...
    {
        double a = s.weekavea(i,si);
        double as = s.weekaves(i,si);
...
//        filter[i] = true;
    ...
    if(1)
    {
        ...
        if(estimate)
        {
            uncertain res;
#ifdef NEW
            clog << "ll= " << s.estimate(params,siest,res,filter,timest) << endl;
#else
